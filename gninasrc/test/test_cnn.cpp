...
void test_set_atom_gradients() {
  ...
  mgrid->batch_transform[0] =
      caffe::MolGridDataLayer<CNNScorer::Dtype>::mol_transform();
  caffe::MolGridDataLayer<CNNScorer::Dtype>::mol_transform& transform =
      mgrid->batch_transform[0];
  vec center(0, 0, 0);
  ...
  caffe::MolGridDataLayer<CNNScorer::Dtype>::mol_transform cpu_transform =
      mgrid->batch_transform[0];
  gmaker.setAtomGradientsCPU(cpu_transform.mol.atoms,
      cpu_transform.mol.whichGrid, cpu_transform.Q.boost(), grids,
      cpu_transform.mol.gradient);

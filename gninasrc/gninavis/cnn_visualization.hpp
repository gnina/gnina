...
struct vis_options
{
  ...
  vis_options() :
      frags_only(false), atoms_only(false), verbose(false), output_files(false), skip_bound_check(
          false), outputdx(false), gpu(0), box_size(23.5)
  {
  }
};
...
private:
  ...
  void add_adjacent_hydrogens(std::unordered_set<std::string> &atoms_to_remove,
      bool isRec);

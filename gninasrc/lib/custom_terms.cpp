  ...
  try {
    for (unsigned i = 0, n = creators.size(); i < n; i++) {
      ...
      if (t != NULL) {
        ...
        term_weights[(unsigned) kind].push_back(
            weight * custom_terms::scaling_factor);
        terms::add(1, t);
        return;
      }
    }
...
flv custom_terms::weights() const {
  ...
  for (int index = LastTermKind - 1; index > BaseTerm; index--) {
    ...
    ret.insert(ret.end(), term_weights[index].begin(),
        term_weights[index].end());
  }
...
void custom_terms::add_terms_from_file(std::istream& in) {
  ...
  while (getline(in, line)) {
    if (line.size() < 1 || line[0] == '#') continue; //support comments and blank lines
...
void custom_terms::print(std::ostream& out) const {
  ...
  for (unsigned i = 0, n = conf_indep_names.size(); i < n; i++) {
    out << std::setw(pad) << term_weights[ConfIndependent][i] << " "
        << conf_indep_names[i] << "\n";
  }
}

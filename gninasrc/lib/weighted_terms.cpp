...
weighted_terms::weighted_terms(const terms* t, const flv& weights)
    : t(t), weights(weights), cutoff_(0), conf_indep_start(0) { // does not own t
  ...
  conf_indep_start = enabled_charge_independent_terms.size()
      ...
      + enabled_distance_additive_terms.size();
}
...
result_components weighted_terms::eval_fast(smt t1, smt t2, fl r) const { // intentionally not checking for cutoff
  ...
  VINA_FOR_IN(i, enabled_charge_independent_terms)
    acc[result_components::TypeDependentOnly] += weights[i]
        * t->charge_independent_terms[enabled_charge_independent_terms[i]].eval(
            t1, t2, r);

  sz offset = enabled_charge_independent_terms.size();
  VINA_FOR_IN(i, enabled_charge_dependent_terms)
    acc +=
        t->charge_dependent_terms[enabled_charge_dependent_terms[i]].eval_components(
            t1, t2, r) * weights[offset + i];

  return acc;
}

//dkoes - evalute da terms here
fl weighted_terms::eval_slow(const atom_base& a, const atom_base& b,
    fl r) const {
  fl acc = 0;

...
void ParseEnvironmentOptions(const char *progName, const char *envvar,
    const char *Overview = 0, bool ReadResponseFiles = false);
...
class Option {
  ...
  public:
    ...
    inline enum ValueExpected getValueExpectedFlag() const {
      ...
      return
          VE ?
              static_cast<enum ValueExpected>(VE) :
    ...
    void setValueExpectedFlag(enum ValueExpected Val) {
      setFlag(Val, ValueMask);
    }
    void setHiddenFlag(enum OptionHidden Val) {
      setFlag(Val, HiddenMask);
    }
    void setFormattingFlag(enum FormattingFlags V) {
      setFlag(V, FormattingMask);
    }
  ...
  protected:
    explicit Option(unsigned DefaultFlags)
        : NumOccurrences(0), Flags(DefaultFlags | NormalFormatting),
            Position(0), AdditionalVals(0), NextRegistered(0), ArgStr(""),
            HelpStr(""), ValueStr("") {
      assert(
          getNumOccurrencesFlag() != 0 && getOptionHiddenFlag() != 0
              && "Not all default flags specified!");
    }
  ...
  public:
    ...
    //
    bool addOccurrence(unsigned pos, const string& ArgName, const string& Value,
        bool MultiArg = false);
...
class ValuesClass {
  ...
  public:
    ...
    template<class Opt>
    void apply(Opt &O) const {
      for (unsigned i = 0, e = static_cast<unsigned>(Values.size()); i != e;
          ++i)
...
struct generic_parser_base {
    ...
    void getExtraOptionNames(vector<const char*> &OptionNames) {
      ...
      // vectored to us.
      if (!hasArgStr) for (unsigned i = 0, e = getNumOptions(); i != e; ++i)
...
class parser : public generic_parser_base {
  protected:
    vector<std::pair<const char *, std::pair<DataType, const char *> > > Values;
  public:
    ...
    bool parse(Option &O, const string& ArgName, const string& Arg,
        DataType &V) {
      string ArgVal;
      if (hasArgStr)
        ArgVal = Arg;
...
class opt : public Option, public opt_storage<DataType, ExternalStorage,
    ...
    virtual bool handleOccurrence(unsigned pos, const string& ArgName,
      ...
      if (Parser.parse(*this, ArgName, Arg, Val)) return true;   // Parse error!
      this->setValue(Val);
...
class bits_storage {
    ...
    static unsigned Bit(const T &V) {
      ...
      assert(
          BitPos < sizeof(unsigned) * CHAR_BIT && "enum exceeds width of bit vector!");
      return 1 << BitPos;
    }
...
class alias : public Option {
    ...
    void done() {
      if (!hasArgStr()) error("cl::alias must have argument name specified!");
      if (AliasFor == 0)

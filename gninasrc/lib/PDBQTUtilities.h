...
bool FindBondedPiece(const std::vector<int>& root,
    const std::vector<int>& branch, unsigned int& root_atom,
    unsigned int& branch_atom, unsigned int& root_atom_rank,
    unsigned int& branch_atom_rank, const OpenBabel::OBMol& mol,
...
bool Separate_preserve_charges(OpenBabel::OBMol & mol,
    std::vector<OpenBabel::OBMol> & result);
unsigned int FindFragments(OpenBabel::OBMol mol,
    std::vector<std::vector<int> >& rigid_fragments, unsigned desired_root,

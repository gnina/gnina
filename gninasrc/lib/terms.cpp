...
unsigned conf_independent_inputs::num_bonded_heavy_atoms(const model& m,
    const atom_index& i) const { // FIXME? - could be static, but I don't feel like declaring function friends
  unsigned acc = 0;
...
unsigned conf_independent_inputs::atom_rotors(const model& m,
    const atom_index& i) const { // the number of rotatable bonds to heavy ligand atoms
  unsigned acc = 0;
  ...
  VINA_FOR_IN(ligand_i, m.ligands) {
    ...
    VINA_RANGE(i, lig.begin, lig.end) {
      ...
      if (!a.is_hydrogen()) {
        ...
        ++num_heavy_atoms;
        if (xs_is_hydrophobic(a.get())) ++num_hydrophobic_atoms;
...
conf_independent_inputs::conf_independent_inputs()
    : num_tors(0), num_rotors(0), num_heavy_atoms(0), num_hydrophobic_atoms(0),
        ligand_max_num_h_bonds(0), num_ligands(0), ligand_lengths_sum(0) {
}
...
sz terms::size_conf_independent(bool enabled_only) const { // number of parameters does not necessarily equal the number of operators
  ...
  VINA_FOR_IN(i, conf_independent_terms)
    if (!enabled_only || conf_independent_terms.enabled[i]) acc +=
        conf_independent_terms[i].size();
  return acc;
}
...
flv terms::evale_robust(const model& m, std::vector<flv>& per_atom) const {
  ...
  VINA_FOR_IN(j, m.grid_atoms) {
    ...
    if (brick_distance_sqr(box_begin, box_end, a.coords) < max_r_cutoff_sqr
        && t < n && !is_hydrogen(t)) // exclude, say, Hydrogens

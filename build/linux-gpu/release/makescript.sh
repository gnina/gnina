...
if [ "$(hostname)" = "t410" ]
then
make -j8 BASE=/usr CUDA_INCLUDE=/opt/cuda/include OPENBABEL_INCLUDE=/usr/include/openbabel-2.0  NVCC=/opt/cuda/bin/nvcc LDFLAGS='-L/usr/lib -L. -L/usr/local/lib -L/opt/cuda/lib64'
else

  ...
  const vector<Blob<Dtype>*>& top)
{
  ...
  caffe_gpu_gemm_batch<Dtype>(CblasNoTrans, CblasNoTrans, M, N, K,
  ...
  const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
{
  ...
  if (propagate_down[0])
  {
    caffe_gpu_gemm_batch<Dtype>(CblasNoTrans, CblasTrans, M, K, N,
      (Dtype) 1., C_diff, B_data, (Dtype) 0., A_diff, batch_size);
  }
  if (propagate_down[1])
  {
    caffe_gpu_gemm_batch<Dtype>(CblasTrans, CblasNoTrans, K, N, M,

...
template<typename Dtype>
void MolGridDataLayer<Dtype>::setAtomGradientsGPU(GridMaker& gmaker,
    Dtype* diff, unsigned batch_size) {
  abort();
  /*
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    gmaker.setCenter(transform.center[0], transform.center[1],
        transform.center[2]);
    ...
    if (nfull_blocks)
      set_atom_gradients<<<nfull_blocks, THREADS_PER_BLOCK>>>(gmaker, atoms,
          whichGrid, gradient,
          make_float3(molcenter[0], molcenter[1], molcenter[2]),
    ...
    if (nthreads_remain)
      set_atom_gradients<<<1, nthreads_remain>>>(gmaker, atoms, whichGrid,
          gradient, make_float3(molcenter[0], molcenter[1], molcenter[2]),
          gpu_q,

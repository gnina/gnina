...
class MolGridDataLayer : public BaseDataLayer<Dtype> {
  public:
    ...
    unsigned getNumGridPoints() const {
    ...
    unsigned getExampleSize() const {
      return example_size;
    }

    unsigned getNumReceptorTypes() const {
      return numReceptorTypes;
    }

    unsigned getNumLigandTypes() const {
      return numLigandTypes;
    }
  ...
  protected:
    ...
    bool ignore_receptor = false; // for gninadream virtual screen, use rec only to set center
    bool use_rec_center = false; // for gninadream virtual screen

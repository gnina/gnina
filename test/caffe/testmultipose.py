...
modelf = open('duplicate.model','w')
modelf.write(model.replace('DUPLICATE','true'))
modelf.close()

modelf = open('noduplicate.model','w')
...
typef = open('input.types','w')
typef.write(types)
typef.close()

net = caffe.Net('duplicate.model',caffe.TRAIN)
res = net.forward()
...
net = caffe.Net('noduplicate.model',caffe.TRAIN)
res = net.forward()
data = res['data']
labels = res['label']

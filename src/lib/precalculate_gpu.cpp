...
void precalculate_gpu::evaluate_splines(const GPUSplineInfo& spInfo,
...
pr precalculate_gpu::eval_deriv(const atom_base& a, const atom_base& b, fl r2) const {
    assert(r2 <= m_cutoff_sqr);
    smt t1 = a.get();
    smt t2 = b.get();
    fl r = sqrt(r2);

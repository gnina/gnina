...
void precalculate_gpu::setup_points(std::vector<std::vector<pr> >& points,
		smt t1,smt t2,double cutoff,unsigned n,const scoring_function& sf) const{
	assert(n >= 2);
...
precalculate_gpu::precalculate_gpu(const scoring_function& sf,fl factor_) : // sf should not be discontinuous, even near cutoff, for the sake of the derivatives
		precalculate(sf), data(num_atom_types(), GPUSplineInfo()), cpudata(
				num_atom_types(), spline_cache()), deviceData(NULL), device_vals(NULL), device_derivs(
